...
- 今日首頁與計畫頁頂部固定顯示今日訓練快捷入口；未完成課表可直接進入開始前確認，傷病限制狀態則先進入課表詳情，不得繞過限制。
- 週課表中的當日整列使用淡綠底、左側綠線與「今天」文字標示，不能只依賴顏色辨識。
- 未完成課表可使用右側把手拖曳至同一週的其他日期，放開後立即樂觀更新並儲存。
- 拖曳可放在同日第一堂之前、任兩堂課表之間或最後，插入順序須持久化並同步套用到今日首頁；同日課表之間保留至少 8 px 間距。
- 課表本體向右滑動 88 px，或滑動速度達 650 px/s 時，開啟刪除二次確認；手勢本身不直接刪除。
- 週課表頂部須提示右滑刪除手勢；所有刪除確認均顯示不可復原警告、紅色確認按鈕與取消動作。
...
- 跑步、單車與鐵人模式開始後顯示「數據／地圖」分頁；地圖只繪製瀏覽器實際取得的 GPS 定位點，定位失敗時不得產生模擬路線。
- GPS 精度差於 60 公尺、時間倒退或移動少於 3 公尺的定位點不加入路徑；暫停訓練時同步停止新增定位點。
- 計時開始後不能再切換模式。
